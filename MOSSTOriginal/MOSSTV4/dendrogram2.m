function [h,T] = dendrogram2(Z,p,grafhandle,fighandle)
%DENDROGRAM Generate dendrogram plot for MOSST.
...
h = plot(A,B,'b','Parent',grafhandle);
set(grafhandle,'XLim',[0 m+2]);
set(grafhandle,'YLim',[0 max(Z(:,3))*1.05]);
%set(gca,'XTickLabel',[],'XTick',[],'box','off','xcolor','w');

%text((1:m)-0.2,zeros(m,1)-0.05*max(Z(:,3)),label);
n = size(label,1);
set(grafhandle, 'Xlim', [.5 n+.5], 'XTick', 1:n, 'XTickLabel', label, ...
         'Box', 'off');
